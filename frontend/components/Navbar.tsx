...
import { Moon, Sun, LogOut, ChevronDown, Loader2, Star } from 'lucide-react';
...
export default function Navbar() {
  ...
  const { address, isConnecting, walletType, connect, disconnect } =
    useWallet();
  ...
  // Fetch reward balance when address changes or on manual refresh
  useEffect(() => {
    const fetchBalance = (force = false) => {
      if (address) {
        getRewardBalance(address, force).then(setRewardBalance);
      } else {
    ...
    window.addEventListener('refresh-rewards', handleRefresh);
    return () => window.removeEventListener('refresh-rewards', handleRefresh);
  }, [address]);
  ...
  return (
    <>
      <nav className="sticky top-0 z-20 flex items-center justify-between px-6 md:px-12 h-15 bg-(--nav-bg) border-b border-(--border) backdrop-blur-md transition-colors duration-300">
                ...
                {dropdownOpen && (
                  ...
                  >
                    <div className="px-4 py-3 border-b border-(--border)">
                      ...
                      <p className="font-mono text-[0.78rem] text-(--text) truncate">
                        {address}
                      </p>
                    </div>
                    <button
        ...
        {isModalOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-(--bg) border border-(--border) w-full max-w-90 rounded-3xl overflow-hidden shadow-2xl shadow-black/40"
            >
              <div className="p-6">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="font-bold text-[0.9rem] text-(--text)">
                        Freighter
                      </p>
                      <p className="text-[0.7rem] text-(--text2)">
                        Browser extension
                      </p>
                    </div>
                  </button>
